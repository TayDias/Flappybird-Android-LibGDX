...
public class MainClass extends ApplicationAdapter {
	...
	private void draw(){
		...
		fonte.draw(batch, String.valueOf(pontos),
				(screenx - getTamx(fonte, String.valueOf(pontos)))/2,
				0.98f*screeny);
	...
	private void update(float time){
		if(estado == 1){
			...
			for(Cano c:canos){
				if(Intersector.overlaps(passaro.corpo, c.colide)){
					Gdx.app.log("Log", "bateeeeu!");
					som.play("hit");
					passaro.perdeu();
					estado = 2;
				}
				if(Intersector.overlaps(passaro.corpo, c.push)){
					Gdx.app.log("Log", "cuidado!");
					som.play("hit");
